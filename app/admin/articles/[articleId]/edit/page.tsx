...
export default async function EditArticlePage({ params }: Props) {
  ...
  const { data: article } = await supabase.from("articles").select("id, title, summary, category, reading_time, cover_image_url, content_blocks, references, status").eq("id", articleId).maybeSingle();
  if (!article) notFound();
